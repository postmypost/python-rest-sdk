...
class Attach(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="Attachment type: image — image; video — video. ")
    __properties: ClassVar[List[str]] = []

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['link', 'image', 'video']):
            raise ValueError("must be one of enum values ('link', 'image', 'video')")
        return value
